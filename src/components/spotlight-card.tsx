...
const glowColorMap = {
  ...
  gold: { base: 45, spread: 50 }, // Approximation for --accent-color: #10b981 which is green but let's use green for default
};
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  const getInlineStyles = () => {
    const baseStyles: any = {
      ...
      '--radius': '14',
      '--border': '1',
      '--backdrop': 'transparent', // using transparent instead of hsl(0 0% 60% / 0.12) to match standard cards
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 210) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 70) * 1%) / var(--bg-spot-opacity, 0.4)), transparent
      )`,
  ...
  const beforeAfterStyles = `
    ...
    [data-glow] > [data-inner-glow]::before {
      inset: -10px;
      border-width: 10px;
    }
  `;
